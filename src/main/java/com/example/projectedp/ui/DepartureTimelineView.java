...
public class DepartureTimelineView extends Region {
    ...
    private static final double WIDTH = 470;     // szerokość osi
    private static final double HEIGHT = 100;    // wysokość komponentu
    private static final double MARGIN = 0;     // margines z lewej i prawej
    ...
    public void setDepartures(List<Departure> departures) {
        ...
        List<Departure> filtered = departures.stream()
                .filter(d -> {
                    ...
                    return diff >= 0 && diff <= MAX_MINUTES;
                })
    ...
    private void drawTimeline() {
        ...
        for (Map.Entry<LocalDateTime, List<Departure>> entry : grouped.entrySet()) {
            LocalDateTime departureTime = entry.getKey();
            List<Departure> group = entry.getValue();
            long minutesFromNow = Duration.between(now, departureTime).toMinutes();

            if (minutesFromNow < 0 || minutesFromNow > MAX_MINUTES) continue;
            ...
            // Etykieta czasu
            Label timeLabel = new Label(departureTime.format(timeFormatter));
            timeLabel.setLayoutX(x - 20);
            timeLabel.setLayoutY(HEIGHT / 2 - 35);
            timeLabel.setStyle("-fx-font-size: 11px; -fx-font-weight: bold;");

            content.getChildren().addAll(circle, timeLabel);

            // Linie autobusowe pod osią, ułożone pionowo
            int i = 0;
            for (Departure d : group) {
                Label lineLabel = new Label(d.getLine());
                lineLabel.setLayoutX(x - 10);
                lineLabel.setLayoutY(HEIGHT / 2 + 15 + (i * 20));
